...
# Live Azure python3 has no pip/gunicorn on PATH (FastAPI cutover used clean
# zip deploys). CI vendors webapp deps into deps/; boot with that interpreter.
if [ -d "${ROOT}/deps" ]; then
  export PYTHONPATH="${ROOT}/deps${PYTHONPATH:+:$PYTHONPATH}"
fi
PY="$(command -v python3)"
GUNICORN_CMD="${PY} -m gunicorn --config=${ROOT}/gunicorn.conf.py --bind=0.0.0.0:${PORT} --workers=${WORKERS} --worker-class=gthread --threads=${THREADS} --timeout=${TIMEOUT} --access-logfile=- --error-logfile=- wsgi:application"

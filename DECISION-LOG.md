...
**Deliberately left (judgment, not oversight):** a couple of inner-function names (`_apply`, `_check`) and the unused `ROLE_ADMIN` constant were flagged as style nits; per the clean-code rule ("don't refactor for the sake of it"), renaming long-stable internals for marginal gain wasn't worth the churn. Litestream stays gated until cutover.

...
| precious import | copies every `views` row + view/layout children and every `report_schedules` row + weekday/monthday/recipient/salesman children; then fills extra JSON backup rows into those same columns (no blobs stored); projected `legacy_id` rows are not duplicated; two schedules on the same view+time both land; admins see Owner + Name |

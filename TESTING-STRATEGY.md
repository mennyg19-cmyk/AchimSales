...
- Personal and company schedule create reject `customer_last_order` (`test_personal_and_company_schedules_reject_customer_last_order`).
...
- CI `git diff --check` uses the empty tree vs `HEAD` (every tracked file), not the working-tree diff. Extra blank lines at EOF fail.

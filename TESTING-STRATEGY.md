...
- Personal wizard stores optional `email_subject` / `email_html` with `{Schedule}` `{SharePointUrl}` `{DownloadButton}` tokens. Blank keeps the auto subject/body. Grid PUT without those keys keeps them. Script tags and `javascript:` / `data:` hrefs are stripped on store, including encoded newlines/tabs. `[TEST]` and the retry-after-failure subject mark stay on a custom subject.

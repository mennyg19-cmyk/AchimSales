...
from webapp.db import (
    get_db, add_notification, get_excluded_customers, get_setting, set_setting,
    upsert_order_headers_cache, upsert_order_lines_cache,
)

log = logging.getLogger(__name__)

from webapp.config import REFRESH_INTERVAL_SECONDS
...
def refresh_cache(salesman_key: str | None = None):
    ...
    try:
        ...
        # -- Order headers cache (individual rows for customer detail page) --
        _refresh_order_headers_cache(headers_df, customers_df, now_str, _step)

        # -- Order lines cache --
        _refresh_order_lines_cache(base_url, token, company, headers_df, now_str, _step)
...
def _refresh_order_headers_cache(headers_df, customers_df, now_str, _step):
    """Store individual order header rows from the already-fetched headers_df."""
    import pandas as pd
    try:
        if headers_df.empty:
            log.info("No order headers to cache")
            return
        _step("Caching order headers…")

        cust_names = {}
        if not customers_df.empty:
            for _, r in customers_df.iterrows():
                cust_names[str(r.get("CustomerAccount", "")).strip()] = str(r.get("CustomerName", "")).strip()

        rows = []
        for _, r in headers_df.iterrows():
            acct = str(r.get("CustomerAccount", "")).strip()
            od = r.get("OrderDate")
            rows.append({
                "sales_order_number": str(r.get("SalesOrderNumber", "")).strip(),
                ...
                "customer_requisition": str(r.get("CustomerRequisition", "")),
                "sales_order_name": str(r.get("SalesOrderName", "")),
                "salesman": str(r.get("Salesman", "")),
                "customer_name": cust_names.get(acct, ""),
                "last_refreshed": now_str,
            })
        upsert_order_headers_cache(rows)
        _step(f"Cached {len(rows)} order headers")
    except Exception:
        log.exception("Order headers cache refresh failed (non-fatal)")


def _refresh_order_lines_cache(base_url, token, company, headers_df, now_str, _step):
    """Fetch order lines for recent orders and cache them.

    Only caches lines for orders from the last 90 days to keep volume manageable.
    """
    ...
    try:
        ...
        _step("Fetching order lines for recent orders…")
        cutoff = (date.today() - timedelta(days=90)).isoformat()
        recent = headers_df.copy()
        if "OrderDate" in recent.columns:
            import pandas as pd
            recent["_od_str"] = pd.to_datetime(recent["OrderDate"], errors="coerce").dt.strftime("%Y-%m-%d")
            recent = recent[recent["_od_str"] >= cutoff]

        order_nums = set(recent["SalesOrderNumber"].astype(str).str.strip().tolist())
        if not order_nums:
            log.info("No recent orders for lines cache")
            return

        log.info("Fetching lines for %d recent orders (last 90 days)", len(order_nums))
        lines_df = fetch_sales_order_lines(base_url, token, order_nums, company_id=company)

        if lines_df.empty:
            log.info("No order lines returned")
            return

        _step(f"Caching {len(lines_df)} order lines…")
        ...
        for _, r in lines_df.iterrows():
            rows.append({
                ...
                "item_number": str(r.get("Item#", "")),
                "line_description": str(r.get("LineDescription", "")),
                "qty_ordered": _safe_float(r.get("QtyOrdered")),
                "sales_price": _safe_float(r.get("SalesPrice")),
                "line_total": _safe_float(r.get("Total")),
                "line_status": str(r.get("RawLineStatus", "")),
                "last_refreshed": now_str,
            })
        upsert_order_lines_cache(rows)
        _step(f"Cached {len(rows)} order lines")
    except Exception:
        log.exception("Order lines cache refresh failed (non-fatal)")

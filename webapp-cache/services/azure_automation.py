...
def start_job(report_name: str, extra_args: str = "",
              webapp_record_id: str = "") -> str:
    ...
    client = get_client()
    job_name = str(_uuid.uuid4())

    parameters = {"report_name": report_name}
    if extra_args:
        parameters["extra_args"] = extra_args
    if webapp_record_id:
        parameters["webapp_record_id"] = webapp_record_id
    ...
    log.info("Started job %s (report=%s, args=%s, record=%s)",
             job_name, report_name, extra_args, webapp_record_id)
    return job_name


def get_job_status(job_name: str) -> dict:
    """Return the current status of an Azure Automation Job.

    Returns dict with keys: status, start_time, end_time, exception.
    Status values: New, Activating, Running, Completed, Failed, Stopped, Suspended.
    """
    client = get_client()
    try:
        job = client.job.get(RESOURCE_GROUP, AUTOMATION_ACCOUNT, job_name)
        return {
        ...
        log.exception("Failed to get job status for %s", job_name)
        return {"status": "Unknown", "start_time": None, "end_time": None, "exception": None}


def stop_job(job_name: str) -> bool:
    """Stop (cancel) a running Azure Automation Job. Returns True on success."""
    client = get_client()
    try:
        client.job.stop(RESOURCE_GROUP, AUTOMATION_ACCOUNT, job_name)
        log.info("Stopped job %s", job_name)
        return True
    except Exception:
        log.exception("Failed to stop job %s", job_name)
        return False

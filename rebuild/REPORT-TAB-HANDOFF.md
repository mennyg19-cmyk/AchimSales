...
For the owner, the DBA, and the next agent. FastAPI is already live. `app/assemble.py` is a **stopgap**. Flask `v3/report_engine` is gone from the tree; formulas below were copied from that code (history `063d9de` and leftover PR #35).
...
Dummy shape: `rebuild/sample-invoiced-response.json`. Top level `data.raw` + `data.tabs.<tab>.rows`.

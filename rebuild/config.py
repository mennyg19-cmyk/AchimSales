    ...
    def validate(self) -> None:
        ...
        if self.is_prod:
            ...
            for label, db_path in (
                ...
                if _is_network_share(db_path):
                    problems.append(f"{label} must be local disk, not a network share: {db_path}")
                elif _is_app_service_home(db_path):
                    problems.append(
                        f"{label} must be on local disk, not the App Service /home share "
                        f"(it's a network share where SQLite's worker coordination breaks): {db_path}"
                    )
            # Litestream is the durable database's backup. It only becomes a hard
            # requirement at cutover; until then the temporary slot can run
            # without it so it can deploy and be reviewed.

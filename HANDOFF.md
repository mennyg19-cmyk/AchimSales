...
- Legacy CSRF on POST/PUT/PATCH/DELETE (Entra callback exempt). Forms use `{% csrf_token %}`; Semgrep's Django rule does not accept that tag so those forms have `nosemgrep`. Fetch still uses the meta token. Feather and v3 Tabulator served locally; unused Chart.js CDN removed. Google Maps stays on Google's CDN.

## What's in progress

- Session revoke and git-history purge are **BLOCKED** (need Azure Flask secret rotation + coordinated history rewrite).
- Remaining review security: a few innerHTML sinks that are static or already escaped. Rebuild (`/test-next`) still loads Tabulator from unpkg.

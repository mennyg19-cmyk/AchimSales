...
Last updated: 2026-09-08 (/test vs beta DB compare skipped)
...
Q8/Q9 as before. Home (`BETA_*`) is the live data. `/test` stays. Azure Automation is not a go-live path. No slot until Standard S1+. No cookie rotation in git. Mounts stay.
...
- Cookie rotation (`FLASK_SECRET_KEY` / `FLASK_SECRET`).
- Phase 10: mark draft ready, whole-branch review, merge to `main`.

## Next action

Owner rotates Flask cookie secrets in Azure (logs everyone out once). Keep draft. No merge.

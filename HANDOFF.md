...
- Legacy CSRF on POST/PUT/PATCH/DELETE (Entra callback exempt). Forms use `{% csrf_token %}`; Semgrep's Django rule does not accept that tag so those forms have `nosemgrep`. Fetch still uses the meta token. Feather served locally; unused Chart.js CDN removed. Google Maps stays on Google's CDN.

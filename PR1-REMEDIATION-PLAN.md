...
- [x] Permit view-only managers to trigger company **Send now**; edit/copy/delete/toggle
  remain `can_edit_master` only. Private (`is_shared=0`) masters 404 unless owner/run-as/
  privileged. Evidence: `test_view_only_manager_can_run_but_not_edit_master_schedule`,
  `test_view_only_manager_cannot_run_private_master_schedule`. Gate closed on `d90f116`.
...
- [x] Add live status/error announcements for admin, dashboard, Settings, schedule sends, and report runs. Evidence: `#addUserMsg`, `#euMsg`, `#esMsg`, `#dashRefreshStatus`, `#exclHint`, `#testModeMsg`, page-level `#runStatus`, and report `#reportStatus` are live regions; errors switch to `role=alert` / assertive. Isolated authenticated browser inspected admin, dashboard, Settings, and schedules on 2026-09-03. Phase 8.15 (`77bdf10`): `#reportStatus` polite live region; errors assertive without wiping Cancel. Source test plus representative CDP matrix (developer, 320/1280, default/dark) 6/6 PASS. Loop A+B zero. Loop C one optional nit left (ternary also in admin/schedules). Trust-boundary N/A. 8.6 F1/F2 closed on `c9c01a7`.

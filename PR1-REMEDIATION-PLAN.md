...
- [x] Complete searchable-picker option navigation and focus return. Evidence:
  shared `SearchablePicker` now supplies Arrow/Home/End, Enter/Space, Escape,
  combobox/listbox/option ARIA, and focus return; local Chrome exercised both
  Settings exclusions and Ordered report customers with two fixture rows.

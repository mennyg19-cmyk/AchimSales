...
def test_main_wraps_retry(monkeypatch):
    calls = []

    def fake_retry(run_fn, **_kw):
        calls.append(run_fn.__name__)
        return 0

    monkeypatch.setattr("universal_runbook.run_with_retry", fake_retry)
    assert main() == 0
    assert calls == ["_job"]

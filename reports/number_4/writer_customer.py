"""
Number 4 Report - By Customer Excel writer (streaming / write_only mode).
...
def write_by_customer(
    ...
    """Write By Customer workbook with 12 Months and Year to Date sheets."""
    log.info("Writing Number 4 By Customer: 12mo=%d rows, YTD=%d rows", len(agg_12), len(agg_ytd))
...
def _write_sheet(ws, agg_df, month_labels, qty_cols, dol_cols):
    ...
    headers = ["Customer #", "Customer Name", "Item #", "Item Name"]
    ...
    for cust_account, grp in agg_df.groupby("CustomerAccount", sort=True):
        cust_name = grp["CustomerName"].iloc[0] if "CustomerName" in grp.columns else ""
        ...
        for _, r in grp.iterrows():
            vals = [r.get("CustomerAccount", ""), r.get("CustomerName", ""),
                    r.get("Item_#", ""), r.get("Item_Name", "")]

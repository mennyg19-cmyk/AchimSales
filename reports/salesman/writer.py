...
def write_salesman_report(
    ...
    """Write a single month tab to a workbook.

    If wb is None, creates a new workbook. Returns the workbook.
    """
    if wb is None:
        wb = Workbook()
        ws = wb.active
    ...
    headers = _build_headers(year, month)
    ...
    row_num = 2
    salesmen = month_data.groupby("Salesman", dropna=False, sort=False)

    for sm_name, grp in salesmen:
        ws.cell(row=row_num, column=1, value=str(sm_name))
        ws.cell(row=row_num, column=1).font = Font(bold=True, size=11)
        row_num += 1

        for _, r in grp.iterrows():
            _write_data_row(ws, row_num, r, year, month)
            row_num += 1

        _write_totals_row(ws, row_num, grp, year, month)
        row_num += 2

    _write_grand_totals(ws, row_num, month_data, year, month)
    _apply_formatting(ws, row_num, len(headers))
...
def _build_headers(year: int, month: int) -> list[str]:
    month_name = calendar.month_name[month]
    last_year = year - 1
    return [
        "Sort Number", "Cust. #", "Customer Name",
        f"Sales {month_name} {year}", f"Sales {month_name} {last_year}",
        "$ This Year to Last Year", "% This Year to Last Year",
        f"Sales {year} Jan Thru {month_name}", f"Sales {last_year} Jan Thru {month_name}",
        "$ This Year to Last Year", "% This Year to Last Year",
        f"Sales Year to Date {year}", f"Sales Year to Date {last_year}",
        "$ This Year to Last Year", "% This Year to Last Year",
    ]
...
def _write_data_row(ws, row_num, r, year, month):
    ws.cell(row=row_num, column=1, value=r.get("Sort Number", ""))
    ws.cell(row=row_num, column=2, value=r.get("Cust. #", ""))
    ws.cell(row=row_num, column=3, value=r.get("Customer Name", ""))
    ...
    for i, (val, band_font) in enumerate(zip(money_vals, band_fonts)):
        c_idx = 4 + i
...
def _write_totals_row(ws, row_num, grp, year, month):
    ws.cell(row=row_num, column=1, value="").fill = FILL_TOTALS
    ws.cell(row=row_num, column=2, value="").fill = FILL_TOTALS
    ws.cell(row=row_num, column=3, value="TOTALS").fill = FILL_TOTALS
    ws.cell(row=row_num, column=3).font = FONT_HEADER
    ...
    for i, col in enumerate(sum_cols):
        ...
        if is_pct:
            base_col = sum_cols[i - 1]
            prior_col = sum_cols[i - 2]
            if base_col and prior_col:
                diff = grp[base_col].sum() if base_col in grp.columns else 0
                prior = grp[prior_col].sum() if prior_col in grp.columns else 0
                cell.value = diff / prior if prior else 0
            cell.number_format = "0.00%"
        elif col and col in grp.columns:
            cell.value = grp[col].sum()
...
def _write_grand_totals(ws, row_num, data, year, month):
    ws.cell(row=row_num, column=3, value="GRAND TOTALS")
    ...
    for i, col in enumerate(sum_cols):
        ...
        if is_pct:
            ...
            prior_col = sum_cols[i - 2]
            if base_col and prior_col:
                diff = data[base_col].sum() if base_col in data.columns else 0
                prior = data[prior_col].sum() if prior_col in data.columns else 0
                cell.value = diff / prior if prior else 0
            cell.number_format = "0.00%"
        elif col and col in data.columns:
            cell.value = data[col].sum()
...
def _apply_formatting(ws, max_row, n_cols):
    if max_row > 1 and n_cols > 0:
        ws.auto_filter.ref = f"A1:{get_column_letter(n_cols)}{max_row}"

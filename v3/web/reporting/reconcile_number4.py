...
# LIVE Number 4 drops free-text docs: FINV-/FCRD- invoice numbers, or no sales order.
_FREE_TEXT_INV_RE = re.compile(r"^(FINV|FCRD)[-_]?", re.IGNORECASE)


def _is_free_text_invoice(row: dict) -> bool:
    """True for free-text invoices/credits Number 4 does not include."""
    inv_no = str(_get(row, "InvoiceNumber", "Invoice", "InvoiceId") or "").strip()
    if inv_no and _FREE_TEXT_INV_RE.match(inv_no):
        return True
    so = str(_get(
        row, "salesorder", "SalesOrder", "SalesOrderNumber", "SalesId",
    ) or "").strip()
    return not so
...
def reconcile(
    ...
    *,
    ...
    for r in inv_rows:
        if _is_free_text_invoice(r):
            free_text_rows += 1
            free_text_sub = round(free_text_sub + _subtotal(r), 2)
            free_text_ti = round(free_text_ti + _total_invoice(r), 2)
            continue
    ...
    return {
        ...
        "invoice_fetch": {
            ...
            "free_text_excluded": {
                "note": "FINV-/FCRD- invoice numbers, or blank sales order (Number 4 LIVE rule).",
                "row_count": free_text_rows,
                "subtotal_sum": free_text_sub,
                "total_invoice_sum": free_text_ti,
            },
        },
        "number4": {

...
DRIFT_LEDGER: tuple[DriftDecision, ...] = (
    DriftDecision("invoiced", "tariff_source",
                  ...
                  rationale="DBA grouping charges at the order level; deferred until SP revision lands."),
    DriftDecision("invoiced", "credit_detection",
                  "Detect credits by substring 'contains' vs invoice-number prefix?",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="User confirmed: live's substring-contains rule is correct."),
    ...
    DriftDecision("ordered", "status_qty_engine",
                  ...
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="SP now returns authoritative QtyShipped + QtyCancelled; WHS/packing-slip derivation no longer needed."),
    DriftDecision("ordered", "amazon_temp_rule",
                  "Amazon 9300/9301 temporary-item special handling.",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="User said SP now has correct data, so temp cancellation rule is no longer needed."),
    DriftDecision("ordered", "error_item_filter",
                  "Exclusion of rows flagged 'ERROR ITEM'.",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="v3 filters ERROR ITEM on item number (matches live). Confirmed working."),
    DriftDecision("number_4", "book_price",
                  "Book Price column source/derivation.",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="Confirmed: Book Price = released_products SalesPrice, joined by ItemNumber. Wired and tested."),
    DriftDecision("number_4", "free_text_exclusion",
                  "Exclusion of free-text (non-item) invoice lines.",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="v3 excludes lines with no sales order (matches live). Confirmed working."),
    DriftDecision("salesman", "group_key_cardinality",
                  "Salesman grouping grain (one row per SalesGroup vs combined).",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="User chose one row per SalesGroup (live behavior). Confirmed working."),
    DriftDecision("customer_activity", "last_order_grain",
                  "Last-order grain: sales header vs sales line.",
                  chosen=DriftChoice.LIVE_ROOT, signed_off=True, owner="menny",
                  rationale="v3 takes max order-date per customer from lines (same result as live). Confirmed working."),
)

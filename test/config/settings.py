"""Configuration for the v2 app (test/).

Plain env-var driven settings. Keep this file small; add things as they're
actually needed.
"""

from __future__ import annotations

import os
from pathlib import Path

TEST_ROOT = Path(__file__).resolve().parent.parent
REPO_ROOT = TEST_ROOT.parent


def _truthy(value: str | None) -> bool:
    return (value or "").strip().lower() in {"1", "true", "yes", "on"}


USE_MOCK_DATA: bool = _truthy(os.environ.get("V2_USE_MOCK_DATA", os.environ.get("USE_MOCK_DATA", "false")))

FLASK_SECRET: str = (
    os.environ.get("V2_FLASK_SECRET")
    or os.environ.get("FLASK_SECRET")
    or os.environ.get("FLASK_SECRET_KEY")  # Azure App Service uses FLASK_SECRET_KEY
    or "change-me-in-prod"
)

# Where to keep writable runtime state on Azure App Service.
#
# IMPORTANT: on Linux App Service, /home/ -- including /home/data/ --
# is Azure Files (SMB), NOT a local SSD. (Old comments in this file
# claimed otherwise and were wrong.) Azure Files is hostile to SQLite
# under any kind of concurrent access:
#   * WAL -shm/-wal sidecar creation fails with "unable to open
#     database file" when another process on the same share holds a
#     write lease.
#   * If a write is interrupted (OOM kill, container restart) while
#     pages are mid-flight, the file can be left "database disk image
#     is malformed" -- recoverable only by .recover.
# Both have happened repeatedly in production.
#
# Fix: SQLite runs on /tmp/ (truly local SSD, exclusive to the
# container, no SMB), with an asynchronous snapshot back to
# /home/data/v2_app.db every few minutes for persistence across
# container restarts. See test/webapp/services/db_sync.py for the
# snapshot loop. /home/data/v2_app.db is the durable source of
# truth; /tmp/v2_app.db is the hot working copy.
_ON_AZURE = bool(os.environ.get("WEBSITE_SITE_NAME"))
_AZURE_PERSISTENT = Path("/home/data") if _ON_AZURE else None

if _AZURE_PERSISTENT is not None:
    try:
        _AZURE_PERSISTENT.mkdir(parents=True, exist_ok=True)
    except Exception:
        _AZURE_PERSISTENT = None

# Persistent (Azure Files, slow + contended): the snapshot target.
APP_DB_PERSISTENT_PATH: Path | None = (
    (_AZURE_PERSISTENT / "v2_app.db") if _AZURE_PERSISTENT is not None else None
)

# Working DB (local SSD on Azure, plain disk locally): what every
# query touches. V2_APP_DB env var overrides for tests.
APP_DB_PATH: Path = Path(
    os.environ.get("V2_APP_DB")
    or ("/tmp/v2_app.db" if _ON_AZURE else str(TEST_ROOT / "app.db"))
)

# Email outbox folder (for test sandbox .eml files)
OUTBOX_DIR: Path = Path(
    os.environ.get("V2_OUTBOX_DIR")
    or (_AZURE_PERSISTENT / "outbox" if _AZURE_PERSISTENT is not None else REPO_ROOT / "test" / "outbox")
)

URL_PREFIX: str = os.environ.get("V2_URL_PREFIX", "/v2")

# ---------------------------------------------------------------------------
# Authentication
# ---------------------------------------------------------------------------
# V2_AUTH_MODE=dev  (default): self-serve dev bypass (any email)
# V2_AUTH_MODE=msal          : Microsoft Entra ID via MSAL auth-code flow
AUTH_MODE: str = (os.environ.get("V2_AUTH_MODE") or "dev").strip().lower()

AZURE_TENANT_ID: str = (
    os.environ.get("V2_AZURE_TENANT_ID")
    or os.environ.get("AZURE_TENANT_ID")
    or os.environ.get("GRAPH_TENANT_ID")  # Azure App Service uses GRAPH_*
    or ""
)
AZURE_CLIENT_ID: str = (
    os.environ.get("V2_AZURE_CLIENT_ID")
    or os.environ.get("AZURE_CLIENT_ID")
    or os.environ.get("GRAPH_CLIENT_ID")
    or ""
)
AZURE_CLIENT_SECRET: str = (
    os.environ.get("V2_AZURE_CLIENT_SECRET")
    or os.environ.get("AZURE_CLIENT_SECRET")
    or os.environ.get("GRAPH_CLIENT_SECRET")
    or ""
)
AUTH_REDIRECT_PATH: str = os.environ.get("V2_AUTH_REDIRECT_PATH", "/auth/callback")

# Comma- or semicolon-separated list of admin emails.
_admin_raw = os.environ.get("V2_ADMIN_EMAILS", "")
ADMIN_EMAILS: set[str] = {
    e.strip().lower()
    for e in _admin_raw.replace(";", ",").split(",")
    if e.strip()
}

# Fallback identity used by code paths without a request context.
DEV_USER_EMAIL: str = os.environ.get("V2_DEV_USER", "dev@local")
